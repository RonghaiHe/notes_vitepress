...
For example, the formation in [Figure 1(a)](#fig-1) is **flexible**. On the other hand, the formation in [Figure 1(f)](#fig-1), which is obtained from the formation in [Figure 1(a)](#fig-1) by **inserting an extra link** $(1, 3)$, is a parallel rigid formation.

Taking the derivative of [$(1)$](#eq-1) (recall that $\boldsymbol{p}$ is a fixed point set and $s(t)$ is time varying in [$(1)$](#eq-1)), we  obtain
$$
...
> [!caution] Theorem 3: generically parallel rigid
...
<figure>
   <img src="./images/2bearing_2-bearing_constraints.jpg" alt="Bearing Constaints v.s. Direction Constraints" width="100%" align="center">
...
<figure>
   <img src="./images/2bearing_3-projection.jpg" alt="Orthogonal Projection Matrix" width="100%" align="center">
...
<figure>
   <img src="./images/2bearing_4-bearing.jpg" alt="Bearing equivalent but not bearing congruent" id="fig-4" width="100%" align="center">
...
> [!tip] Lemma 4
> The **bearing rigidity matrix** $R_\mathcal{B}(\boldsymbol{p})$ in [$(2)$](#eq-2) can be expressed as
>
> <span id="eq-3"></span>
>
...
> [!caution] Theorem 7: Condition for Bearing Rigidity
...
> [!caution] Theorem 8: Condition for Infinitesimal Bearing Rigidity (Algebraic property)
...
> [!caution] Theorem 11: Invariance to Dimension
...
<figure>
   <img src="./images/2bearing_5-not_inf.jpg" alt="not inf" id="fig-5" width="100%" align="center">
...
[Figure 4](#fig-4) shows examples of non-infinitesimal bearing rigid frameworks. The frameworks in [Figure 4](#fig-4) are not infinitesimally bearing rigid because **there exist non-trivial infinitesimal bearing motions** (see, for example, the red arrows). Figure 5 shows some two- and three-dimensional infinitesimally bearing rigid frameworks. It can be verified that each of the frameworks satisfies $\operatorname{rank}(R_\mathcal{B}(\boldsymbol{p})) = dn − d − 1$.

<figure>
   <img src="./images/2bearing_6-inf.jpg" alt="inf" id="fig-6" width="100%" align="center">
...
<figure>
   <img src="./images/2bearing_7-generic.jpg" alt="generic" width="100%" align="center">
...
> [!caution] Theorem 12: Generic Bearing Rigidity of Laman Graphs
> If $\mathcal{G}$ is a **[Laman graph](./1distance#def-laman)**, then $\mathcal{G}$ is **generically bearing rigid** in $\mathbb{R}^d$ for any $d \geqslant 2$.
...
While [Theorem 12](#thm-12) indicates that Laman graphs are generically bearing rigid, a natural question that follows is whether generically bearing rigid graphs are also Laman. The answer may be negative. A counterexample is given in [Figure 9](#fig-9).

<figure>
   <img src="./images/2bearing_9-counterpart.jpg" alt="counterpart" id="fig-9" width="100%" align="center">
...
It immediately follows from [$(5)$](#eq-5) that $R_\mathcal{B}(\boldsymbol{p})\delta\boldsymbol{p} = \mathbf{0}$ if and only if $R_\mathcal{D}(\boldsymbol{p})\delta\boldsymbol{p}^\perp = \mathbf{0}$. <div style="text-align: right;">$\square$</div>
</details>

Given a framework in $\mathbb{R}^2$, Corollary 14-1 suggests that **for any** infinitesimal bearing motion, **there exists** a perpendicular infinitesimal distance motion, and the converse is also true. Corollary 14-1 is illustrated by [Figure 5](#fig-5) (indicated by the red (solid) and blue (dashed) arrows).
...
The formula of [$(7)$](#eq-7) gives the following set inclusion of null spaces
> [!caution] Theorem 15
...
Based on this formula [$(9)$](#eq-9), we give a **necessary and sufficient condition** to guarantee bearing persistence.
...
This necessary and sufficient condition follows from the condition of infinitesimally bearing rigidity in [Theorem 8](#thm-8) and the null space property of $L_\mathcal{B}=\tilde{J}^\top_\mathcal{B}\tilde{R}_\mathcal{B}$ as in [$(9)$](#eq-9)  by applying [Lemma 12](#lem-12). <div style="text-align: right;">$\square$</div>
</details>
...
This conjecture is **not** true. For counterexamples, see the formation graphs in [Figure 10](#fig-10) which will be discussed later. Note that the entries of the bearing Laplacian (and thus its eigenvalue locations) depend on the configurations (agents’ positions). For a given bearing Laplacian associated with a cyclic directed graph and under some special positions, eigenvalues with negative real parts can occur.
...
We note that this condition is not necessary. For a counterexample, see the graph (c) in [Figure 10](#fig-10). In this example, agent $2$ has three out-going edges while the bearing formation of [Figure 10](#fig-10)(c) satisfies $\operatorname{Null}(R_\mathcal{B}) = \operatorname{Null}(L_\mathcal{B})$ and still is **bearing equivalent**.
...
[Lemma 6](#lem-6) is fundamental for the next definition that constitutes a key concept in rigidity theory.
...
Observing [$(11)$](#eq-11), we embed $ \bar{\mathcal{M}}$ into $\mathbb{S}^{2m}$ and, according to Definition, the **bearing function** can be expressed as
...
According to Definition, the extended bearing rigidity matrix $R^+_\mathcal{B}(\boldsymbol{p}(t))$ has to map the vector $\boldsymbol{\delta}^+\in\mathbb{R}^{6n}$ to the derivative of the bearing function. By applying the product rule to [$(12)$](#eq-12), it follows that
...
In light of [$(13)$](#eq-13), it is possible to distinguish the measurements variations induced by the variations of the agents position $\boldsymbol{\delta}_p$ or attitude $\boldsymbol{\delta}_a$:  the former contribution refers to the $1^\text{st}$ and $3^\text{rd}$ addendum in [$(16)$](#eq-16), while the latter contribution is related to the $2^\text{nd}$ one. Accounting for the agents dynamics when embedded in $\mathrm{SE}(3)$, [$(16)$](#eq-16) can then be rearranged as
...
where $\mathbf{U} \bar{\mathbf{E}}^\top, \mathbf{V}\bar{\mathbf{E}}_{o}^\top \in \mathbb{R}^{3m \times 3n}$ link the bearing measurements variations to the agents variations. In detail, the $1^\text{st}$ addendum in [$(17)$](#eq-17) includes the product of three matrices: the matrix $\bar{\mathbf{E}}^\top$ translates the variations of the agents position into relative positions (which are scaled bearings),
the block matrix $\mathbf{U}$ embeds the variation of each bearing in the manifold $\mathbb{S}^{2}$ and, finally, the block matrix $\mathbf{D}_p(t)$ accounts for the projection of each bearing $\boldsymbol{b}_{ij}$ in the $i$-th agent local frame. An analogous reasoning can be carried out for the $2^\text{nd}$ addendum in [$(17)$](#eq-17) where $\mathbf{D}_a(t)$ acts similarly to $\mathbf{D}_p(t)$ since the skew matrix of the relative position vector is an orthogonal matrix. <div style="text-align: right;">$\square$</div>
</details>

The table below shows particularization of the structure of the **extended bearing matrix** in [$(15)$](#eq-15) for the differential manifolds:
