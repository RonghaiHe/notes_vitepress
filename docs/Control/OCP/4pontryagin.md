...
> Here we use a formula from *Optimal control systems* (2.7.32) to conclude them:
> $$\left[H^* + \frac{\partial g}{\partial t}\right]_{T}\delta T + \left[\left(\frac{\partial g}{\partial \boldsymbol{x}}\right)_* - \boldsymbol{p}^*(t)\right]'_{T}\delta \boldsymbol{x}(T) = 0$$
> - If **free** time: $T$ unspecified, hence $\delta T$ is free, then $\left[H^* + \frac{\partial g}{\partial t}\right]_{T} = 0$
> - If **free** endpoint: $x_i(T)$ is free, then $\delta x_i(T)$ is arbitary, then $p_i^*(T) = \left(\frac{\partial g}{\partial {x}_i}\right)_T = 0$
