...
Recall Theorem 1.5 in [§1](./1intro#1-4-linear-combination-and-systems-of-linear-equations) that the relation between subspace and span.
...
As in [§1.6](./1intro#1-6-bases-and-dimension "bases and dimension"), we measure the "size" of a subspace by its dimension. The null space and range are so important that we attach special names to their respective dimensions.
